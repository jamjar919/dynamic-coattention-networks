...
def get_feed_dict(question_batch, context_batch, answer_batch, dropout_keep_prob, index2embedding) :
    ...
    return {
                ...
                graph.get_tensor_by_name("answer_start_true_ph:0"): answer_batch,
...
def build_classifier(embedding):
    ...
    has_answer_ph = tf.placeholder(dtype=tf.int32,shape=[None], name='has_answer_ph')
    ...
    out = tf.tanh(tf.matmul(out, W3_batch) + b3_batch , name = "classifier_output")
    loss = tf.nn.softmax_cross_entropy_with_logits(labels = has_answer_ph, logits = out, name = "loss_v2_classifier")
