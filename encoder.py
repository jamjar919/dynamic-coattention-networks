...
def encoder(questions,contexts,embedding,hidden_unit_size=200):
    ...
    context_encoding, _ = tf.nn.dynamic_rnn(lstm_enc, transpose(context_embedding), dtype=tf.float32, sequence_length = length(context_embedding))
    ...
    question_encoding, _ = tf.nn.dynamic_rnn(lstm_enc, transpose(question_embedding), dtype=tf.float32, sequence_length = length(question_embedding))
    ...
    u_states, _ = tf.nn.bidirectional_dynamic_rnn(cell_bw=cell_bw,cell_fw=cell_fw,dtype=tf.float32,inputs= transpose(C), sequence_length =length(C))
